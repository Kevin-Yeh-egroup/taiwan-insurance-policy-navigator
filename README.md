...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 49 attempted manual batches, 1 waiting on captcha, 48 indexed manual batches, 48 complete manual batches, 67,386 imported TII policy records, 67,141 saved detail pages, and 245 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `49` attempted manual batches, `1` waiting on captcha, `48 / 306` indexed manual batches, `48 / 306` complete manual batches, `67,386` imported TII policy records, `67,141 / 67,386` saved detail pages, and `245` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-048`. Latest completed batches: `tii-property-047` completed with `1,289 / 1,289` official rows, `1,289` product cards, and `1,287 / 1,289` saved detail pages, with `2` detail pages marked for later backfill; `tii-property-048` completed with `3,630 / 3,630` official rows, `2,906` product cards, `724` duplicate official product-id rows, and `2,872 / 2,906` saved detail pages, with `34` detail pages marked for later backfill. `tii-property-049` is prepared and waiting for a fresh human-entered captcha. The remaining `258` TII batches still require fresh operator captcha sessions.
