...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 22 attempted manual batches, 1 waiting on captcha, 21 indexed manual batches, 21 complete manual batches, 32,327 imported TII policy records, 32,209 saved detail pages, and 118 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `22` attempted manual batches, `1` waiting on captcha, `21 / 306` indexed manual batches, `21 / 306` complete manual batches, `32,327` imported TII policy records, `32,209 / 32,327` saved detail pages, and `118` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-021`. Latest completed batches: `tii-property-020` completed by official row coverage with `5,159 / 5,159` official rows, `3,874` deduplicated product cards, `1,285` official duplicate product-id rows, and `3,835 / 3,874` saved detail pages; `tii-property-021` completed by official row coverage with `1,173 / 1,173` official rows, `1,163` deduplicated product cards, `10` official duplicate product-id rows, and `1,163 / 1,163` saved detail pages. `tii-property-022` is prepared and waiting for a fresh human-entered captcha. The remaining `285` TII batches still require fresh operator captcha sessions.
