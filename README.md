...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 35 attempted manual batches, 1 waiting on captcha, 34 indexed manual batches, 34 complete manual batches, 52,624 imported TII policy records, 52,436 saved detail pages, and 188 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `35` attempted manual batches, `1` waiting on captcha, `34 / 306` indexed manual batches, `34 / 306` complete manual batches, `52,624` imported TII policy records, `52,436 / 52,624` saved detail pages, and `188` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-034`. Latest completed batches: `tii-property-033` completed with `1,180 / 1,180` official rows, `1,180` product cards, and `1,180 / 1,180` saved detail pages; `tii-property-034` completed with `888 / 888` official rows, `888` product cards, and `879 / 888` saved detail pages, with `9` detail pages marked for later backfill. `tii-property-035` is prepared and waiting for a fresh human-entered captcha. The remaining `272` TII batches still require fresh operator captcha sessions.
