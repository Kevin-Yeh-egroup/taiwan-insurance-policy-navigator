...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 60 attempted manual batches, 1 waiting on captcha, 59 indexed manual batches, 59 complete manual batches, 79,734 imported TII policy records, 78,553 saved detail pages, and 1,181 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `60` attempted manual batches, `1` waiting on captcha, `59 / 306` indexed manual batches, `59 / 306` complete manual batches, `79,734` imported TII policy records, `78,553 / 79,734` saved detail pages, and `1,181` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-059`. Latest completed batches: `tii-property-058` completed with `577 / 577` official rows, `577` product cards, and `575 / 577` saved detail pages, with `2` detail pages marked for later backfill; `tii-property-059` completed with `623 / 623` official rows, `623` product cards, and `620 / 623` saved detail pages, with `3` detail pages marked for later backfill. `tii-property-060` is prepared and waiting for a fresh human-entered captcha. The remaining `247` TII batches still require fresh operator captcha sessions.
