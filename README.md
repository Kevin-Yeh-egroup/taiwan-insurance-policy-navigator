...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 5 attempted manual batches, 1 waiting on captcha, 4 indexed manual batches, 4 complete manual batches, 4,186 imported TII policy records, and 4,181 saved detail pages.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `5` attempted manual batches, `1` waiting on captcha, `4 / 306` indexed manual batches, `4 / 306` complete manual batches, and `4,186` imported TII policy records. `tii-property-001` is complete with `952 / 952` records; `tii-property-002` is complete with `618 / 618` records and `617` saved detail pages; `tii-property-003` is complete with `525 / 525` records and `525` saved detail pages; `tii-property-004` is complete by official row coverage with `2,667 / 2,667` official rows, `2,091` deduplicated product cards, `576` official duplicate product-id rows, and `2,087` saved detail pages. `tii-property-005` is prepared and waiting for a fresh human-entered captcha. The remaining `302` TII batches still require fresh operator captcha sessions.
