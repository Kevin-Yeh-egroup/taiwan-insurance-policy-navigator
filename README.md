...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 37 attempted manual batches, 1 waiting on captcha, 36 indexed manual batches, 36 complete manual batches, 54,949 imported TII policy records, 54,760 saved detail pages, and 189 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `37` attempted manual batches, `1` waiting on captcha, `36 / 306` indexed manual batches, `36 / 306` complete manual batches, `54,949` imported TII policy records, `54,760 / 54,949` saved detail pages, and `189` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-036`. Latest completed batches: `tii-property-035` completed with `488 / 488` official rows, `488` product cards, and `487 / 488` saved detail pages, with `1` detail page marked for later backfill; `tii-property-036` completed with `2,163 / 2,163` official rows, `1,837` product cards, `326` duplicate official product-id rows, and `1,837 / 1,837` saved detail pages. `tii-property-037` is prepared and waiting for a fresh human-entered captcha. The remaining `270` TII batches still require fresh operator captcha sessions.
