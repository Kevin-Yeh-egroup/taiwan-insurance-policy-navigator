...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 54 attempted manual batches, 1 waiting on captcha, 53 indexed manual batches, 53 complete manual batches, 73,052 imported TII policy records, 71,881 saved detail pages, and 1,171 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `54` attempted manual batches, `1` waiting on captcha, `53 / 306` indexed manual batches, `53 / 306` complete manual batches, `73,052` imported TII policy records, `71,881 / 73,052` saved detail pages, and `1,171` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-053`. Latest completed batches: `tii-property-052` completed with `2,359 / 2,359` official rows, `1,793` product cards, `566` duplicate official product-id rows, and `1,790 / 1,793` saved detail pages, with `3` detail pages marked for later backfill; `tii-property-053` completed with `1,013 / 1,013` official rows, `1,013` product cards, and `93 / 1,013` saved detail pages, with `920` detail pages marked for later backfill. `tii-property-054` is prepared and waiting for a fresh human-entered captcha. The remaining `253` TII batches still require fresh operator captcha sessions.
