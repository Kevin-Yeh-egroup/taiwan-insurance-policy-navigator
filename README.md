...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 56 attempted manual batches, 1 waiting on captcha, 55 indexed manual batches, 55 complete manual batches, 74,443 imported TII policy records, 73,271 saved detail pages, and 1,172 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `56` attempted manual batches, `1` waiting on captcha, `55 / 306` indexed manual batches, `55 / 306` complete manual batches, `74,443` imported TII policy records, `73,271 / 74,443` saved detail pages, and `1,172` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-055`. Latest completed batches: `tii-property-054` completed with `538 / 538` official rows, `538` product cards, and `538 / 538` saved detail pages; `tii-property-055` completed with `853 / 853` official rows, `853` product cards, and `852 / 853` saved detail pages, with `1` detail page marked for later backfill. `tii-property-056` is prepared and waiting for a fresh human-entered captcha. The remaining `251` TII batches still require fresh operator captcha sessions.
