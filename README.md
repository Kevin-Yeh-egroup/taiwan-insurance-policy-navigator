...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 26 attempted manual batches, 1 waiting on captcha, 25 indexed manual batches, 25 complete manual batches, 36,435 imported TII policy records, 36,314 saved detail pages, and 121 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `26` attempted manual batches, `1` waiting on captcha, `25 / 306` indexed manual batches, `25 / 306` complete manual batches, `36,435` imported TII policy records, `36,314 / 36,435` saved detail pages, and `121` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-025`. Latest completed batches: `tii-property-024` completed by official row coverage with `1,785 / 1,785` official rows, `1,400` deduplicated product cards, `385` official duplicate product-id rows, and `1,400 / 1,400` saved detail pages; `tii-property-025` completed with `1,156 / 1,156` official rows, `1,156` product cards, and `1,156 / 1,156` saved detail pages. `tii-property-026` is prepared and waiting for a fresh human-entered captcha. The remaining `281` TII batches still require fresh operator captcha sessions.
