...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 61 attempted manual batches, 1 waiting on captcha, 60 indexed manual batches, 60 complete manual batches, 81,681 imported TII policy records, 80,497 saved detail pages, and 1,184 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `61` attempted manual batches, `1` waiting on captcha, `60 / 306` indexed manual batches, `60 / 306` complete manual batches, `81,681` imported TII policy records, `80,497 / 81,681` saved detail pages, and `1,184` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-060`. Latest completed batches: `tii-property-059` completed with `623 / 623` official rows, `623` product cards, and `620 / 623` saved detail pages, with `3` detail pages marked for later backfill; `tii-property-060` completed with `2,390 / 2,390` official rows, `1,947` product cards, `443` official duplicate product-id rows, and `1,944 / 1,947` saved detail pages, with `3` detail pages marked for later backfill. `tii-property-061` is prepared and waiting for a fresh human-entered captcha. The remaining `246` TII batches still require fresh operator captcha sessions.
