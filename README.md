...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `45` attempted manual batches, `1` waiting on captcha, `44 / 306` indexed manual batches, `44 / 306` complete manual batches, `61,316` imported TII policy records, `61,112 / 61,316` saved detail pages, and `204` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-044`. Latest completed batches: `tii-property-043` completed with `731 / 731` official rows, `731` product cards, and `731 / 731` saved detail pages; `tii-property-044` completed with `3,621 / 3,621` official rows, `2,801` product cards, `820` duplicate official product-id rows, and `2,793 / 2,801` saved detail pages, with `8` detail pages marked for later backfill. `tii-property-045` is prepared and waiting for a fresh human-entered captcha. The remaining `262` TII batches still require fresh operator captcha sessions.
