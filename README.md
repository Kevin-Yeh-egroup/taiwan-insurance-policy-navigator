...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 34 attempted manual batches, 1 waiting on captcha, 33 indexed manual batches, 33 complete manual batches, 51,736 imported TII policy records, 51,557 saved detail pages, and 179 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `34` attempted manual batches, `1` waiting on captcha, `33 / 306` indexed manual batches, `33 / 306` complete manual batches, `51,736` imported TII policy records, `51,557 / 51,736` saved detail pages, and `179` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-033`. Latest completed batches: `tii-property-032` completed with `6,486 / 6,486` official rows, `4,543` product cards, `1,943` duplicate official product-id rows, and `4,531 / 4,543` saved detail pages, with `12` detail pages marked for later backfill; `tii-property-033` completed with `1,180 / 1,180` official rows, `1,180` product cards, and `1,180 / 1,180` saved detail pages. `tii-property-034` is prepared and waiting for a fresh human-entered captcha. The remaining `273` TII batches still require fresh operator captcha sessions.
