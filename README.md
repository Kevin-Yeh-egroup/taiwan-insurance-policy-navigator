...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `46` attempted manual batches, `1` waiting on captcha, `45 / 306` indexed manual batches, `45 / 306` complete manual batches, `62,514` imported TII policy records, `62,310 / 62,514` saved detail pages, and `204` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-045`. Latest completed batches: `tii-property-044` completed with `3,621 / 3,621` official rows, `2,801` product cards, `820` duplicate official product-id rows, and `2,793 / 2,801` saved detail pages, with `8` detail pages marked for later backfill; `tii-property-045` completed with `1,198 / 1,198` official rows, `1,198` product cards, and `1,198 / 1,198` saved detail pages. `tii-property-046` is prepared and waiting for a fresh human-entered captcha. The remaining `261` TII batches still require fresh operator captcha sessions.
