...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 4 attempted manual batches, 1 waiting on captcha, 3 indexed manual batches, 3 complete manual batches, 2,095 imported TII policy records, and 2,094 saved detail pages.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `4` attempted manual batches, `1` waiting on captcha, `3 / 306` indexed manual batches, `3 / 306` complete manual batches, and `2,095` imported TII policy records. `tii-property-001` is complete with `952 / 952` records; `tii-property-002` is complete with `618 / 618` records and `617` saved detail pages; `tii-property-003` is complete with `525 / 525` records and `525` saved detail pages. The remaining `303` TII batches still require fresh operator captcha sessions.
