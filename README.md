...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 62 attempted manual batches, 1 waiting on captcha, 61 indexed manual batches, 61 complete manual batches, 83,030 imported TII policy records, 81,845 saved detail pages, and 1,185 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `62` attempted manual batches, `1` waiting on captcha, `61 / 306` indexed manual batches, `61 / 306` complete manual batches, `83,030` imported TII policy records, `81,845 / 83,030` saved detail pages, and `1,185` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-061`. Latest completed batches: `tii-property-060` completed with `2,390 / 2,390` official rows, `1,947` product cards, `443` official duplicate product-id rows, and `1,944 / 1,947` saved detail pages, with `3` detail pages marked for later backfill; `tii-property-061` completed with `1,349 / 1,349` official rows, `1,349` product cards, and `1,348 / 1,349` saved detail pages, with `1` detail page marked for later backfill. `tii-property-062` is prepared and waiting for a fresh human-entered captcha. The remaining `245` TII batches still require fresh operator captcha sessions.
