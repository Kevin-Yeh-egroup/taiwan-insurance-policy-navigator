...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 31 attempted manual batches, 1 waiting on captcha, 30 indexed manual batches, 30 complete manual batches, 44,267 imported TII policy records, 44,100 saved detail pages, and 167 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `31` attempted manual batches, `1` waiting on captcha, `30 / 306` indexed manual batches, `30 / 306` complete manual batches, `44,267` imported TII policy records, `44,100 / 44,267` saved detail pages, and `167` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-030`. Latest completed batches: `tii-property-029` completed with `1,040 / 1,040` official rows, `1,040` product cards, and `1,040 / 1,040` saved detail pages; `tii-property-030` completed with `1,259 / 1,259` official rows, `1,259` product cards, and `1,226 / 1,259` saved detail pages, with `33` detail pages marked for later backfill. `tii-property-031` is prepared and waiting for a fresh human-entered captcha. The remaining `276` TII batches still require fresh operator captcha sessions.
