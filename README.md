...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 44 attempted manual batches, 1 waiting on captcha, 43 indexed manual batches, 43 complete manual batches, 58,515 imported TII policy records, 58,319 saved detail pages, and 196 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `44` attempted manual batches, `1` waiting on captcha, `43 / 306` indexed manual batches, `43 / 306` complete manual batches, `58,515` imported TII policy records, `58,319 / 58,515` saved detail pages, and `196` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-043`. Latest completed batches: `tii-property-042` completed with `1,610 / 1,610` official rows, `1,610` product cards, and `1,603 / 1,610` saved detail pages, with `7` detail pages marked for later backfill; `tii-property-043` completed with `731 / 731` official rows, `731` product cards, and `731 / 731` saved detail pages. `tii-property-044` is prepared and waiting for a fresh human-entered captcha. The remaining `263` TII batches still require fresh operator captcha sessions.
