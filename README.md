...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 25 attempted manual batches, 1 waiting on captcha, 24 indexed manual batches, 24 complete manual batches, 35,279 imported TII policy records, 35,158 saved detail pages, and 121 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `25` attempted manual batches, `1` waiting on captcha, `24 / 306` indexed manual batches, `24 / 306` complete manual batches, `35,279` imported TII policy records, `35,158 / 35,279` saved detail pages, and `121` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-024`. Latest completed batches: `tii-property-023` completed with `846 / 846` official rows, `846` product cards, and `844 / 846` saved detail pages; `tii-property-024` completed by official row coverage with `1,785 / 1,785` official rows, `1,400` deduplicated product cards, `385` official duplicate product-id rows, and `1,400 / 1,400` saved detail pages. `tii-property-025` is prepared and waiting for a fresh human-entered captcha. The remaining `282` TII batches still require fresh operator captcha sessions.
