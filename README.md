...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 36 attempted manual batches, 1 waiting on captcha, 35 indexed manual batches, 35 complete manual batches, 53,112 imported TII policy records, 52,923 saved detail pages, and 189 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `36` attempted manual batches, `1` waiting on captcha, `35 / 306` indexed manual batches, `35 / 306` complete manual batches, `53,112` imported TII policy records, `52,923 / 53,112` saved detail pages, and `189` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-035`. Latest completed batches: `tii-property-034` completed with `888 / 888` official rows, `888` product cards, and `879 / 888` saved detail pages, with `9` detail pages marked for later backfill; `tii-property-035` completed with `488 / 488` official rows, `488` product cards, and `487 / 488` saved detail pages, with `1` detail page marked for later backfill. `tii-property-036` is prepared and waiting for a fresh human-entered captcha. The remaining `271` TII batches still require fresh operator captcha sessions.
