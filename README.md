...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `1` attempted manual batch, `0` waiting on captcha, `1 / 306` indexed manual batches, `1 / 306` complete manual batches, and `952` imported TII policy records. `tii-property-001` is complete: `952 / 952` records imported, all `96` result pages saved, and `952` detail pages saved. The remaining `305` TII batches still require fresh operator captcha sessions.
