...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 40 attempted manual batches, 1 waiting on captcha, 39 indexed manual batches, 39 complete manual batches, 55,003 imported TII policy records, 54,814 saved detail pages, and 189 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `40` attempted manual batches, `1` waiting on captcha, `39 / 306` indexed manual batches, `39 / 306` complete manual batches, `55,003` imported TII policy records, `54,814 / 55,003` saved detail pages, and `189` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-039`. Latest completed batches: `tii-property-038` completed with `24 / 24` official rows, `24` product cards, and `24 / 24` saved detail pages; `tii-property-039` completed with `5 / 5` official rows, `5` product cards, and `5 / 5` saved detail pages. `tii-property-040` is prepared and waiting for a fresh human-entered captcha. The remaining `267` TII batches still require fresh operator captcha sessions.
