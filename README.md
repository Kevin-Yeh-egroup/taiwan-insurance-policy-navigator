...
- TII manual batches follow the site split: 108 property-insurance batches and 198 life/personal-insurance batches, 306 total. Current TII execution/import status is 29 attempted manual batches, 1 waiting on captcha, 28 indexed manual batches, 28 complete manual batches, 41,968 imported TII policy records, 41,834 saved detail pages, and 134 detail pages marked for later backfill.
...
Current TII manual matrix: `27` property insurers x `4` property categories = `108` batches; `33` life/personal insurers x `6` personal-insurance categories = `198` batches; total `306` manual batches. These batches are shown on the site as clickable property/life groups, but captcha still requires human input. Current status is `29` attempted manual batches, `1` waiting on captcha, `28 / 306` indexed manual batches, `28 / 306` complete manual batches, `41,968` imported TII policy records, `41,834 / 41,968` saved detail pages, and `134` detail pages marked for later backfill. Completed range: `tii-property-001` through `tii-property-028`. Latest completed batches: `tii-property-027` completed with `1,462 / 1,462` official rows, `1,462` product cards, and `1,455 / 1,462` saved detail pages; `tii-property-028` completed by official row coverage with `4,285 / 4,285` official rows, `3,461` deduplicated product cards, `824` official duplicate product-id rows, and `3,460 / 3,461` saved detail pages. `tii-property-029` is prepared and waiting for a fresh human-entered captcha. The remaining `278` TII batches still require fresh operator captcha sessions.
