...
- 目前 TII 人工批次已啟動：`19 / 306`。
- 目前等待驗證碼批次：`1`。
- 目前 TII 已索引批次：`18 / 306`。
- 目前 TII 完整批次：`18 / 306`。
- 目前已匯入 TII 保單結果：`25,714` 筆。
- 目前已保存 TII 明細頁：`25,642` 筆。
...
- `tii-property-019` 已準備好驗證碼，等待下一次人工輸入後執行。
- 剩餘 `288` 個 TII 批次仍需透過 operator 逐批人工輸入驗證碼，送出後由系統自動翻頁、抓明細、匯入。
