...
- 目前 TII 人工批次已啟動：`32 / 306`。
- 目前等待驗證碼批次：`1`。
- 目前 TII 已索引批次：`31 / 306`。
- 目前 TII 完整批次：`31 / 306`。
- 目前已匯入 TII 保單結果：`46,013` 筆。
- 目前已保存 TII 明細頁：`45,846 / 46,013` 筆。
- 目前待補 TII 明細頁：`167` 筆。
...
- `tii-property-029` 官方結果總數為 `1,040` 筆，目前已保存全部 `104` 個結果頁與 `1,040` 份明細頁。
- `tii-property-030` 官方結果總數為 `1,259` 筆，目前已保存全部 `126` 個結果頁與 `1,226` 份明細頁；另有 `33` 份官方明細頁在同一 session 回傳無效明細。
- `tii-property-031` 官方結果總數為 `1,746` 筆，目前已保存全部 `175` 個結果頁與 `1,746` 份明細頁。
- `tii-property-032` 已準備好驗證碼，等待下一次人工輸入後執行。
- 剩餘 `275` 個 TII 批次仍需透過 operator 逐批人工輸入驗證碼，送出後由系統自動翻頁、抓明細、匯入。
