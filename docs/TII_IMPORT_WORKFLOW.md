...
- 目前 TII 人工批次已啟動：`13 / 306`。
- 目前等待驗證碼批次：`1`。
- 目前 TII 已索引批次：`12 / 306`。
- 目前 TII 完整批次：`12 / 306`。
- 目前已匯入 TII 保單結果：`9,831` 筆。
- 目前已保存 TII 明細頁：`9,818` 筆。
...
- `tii-property-013` 已準備好驗證碼，等待下一次人工輸入後執行。
- 剩餘 `294` 個 TII 批次仍需透過 operator 逐批人工輸入驗證碼，送出後由系統自動翻頁、抓明細、匯入。
