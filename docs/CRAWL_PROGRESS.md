...
Updated at: `2026-06-08T09:40:00+08:00`
...
- Attempted TII manual batches: `29 / 306`
- Waiting on captcha: `1`
- Indexed TII manual batches: `28 / 306`
- Completed TII manual batches: `28 / 306`
- Partial TII manual batches: `0`
- Imported TII policy records: `41,968`
- TII official result rows before productId deduplication: `47,889`
- Official duplicate productId rows: `5,921`
- Saved TII detail pages: `41,834 / 41,968`
- Detail pages marked for later backfill: `134`
- Same-company same-name multi-product groups: `1,283` groups / `3,431` cards; these are preserved as separate cards because name-only deduplication would merge different versions.
- Completed TII batches: `tii-property-001` through `tii-property-028`.
- Latest completed TII batch: `tii-property-028` is complete by official row coverage with `4,285 / 4,285` official rows, `3,461` deduplicated product cards, `824` official duplicate product-id rows, and `3,460 / 3,461` saved detail pages; `1` official detail page returned an invalid detail response and is marked for later backfill.
- Previous completed TII batch: `tii-property-027` is complete by official row coverage with `1,462 / 1,462` official rows, `1,462` product cards, and `1,455 / 1,462` saved detail pages; `7` official detail pages returned invalid detail responses and are marked for later backfill.
- Current waiting TII batch: `tii-property-029` is prepared and waiting for a fresh human-entered captcha.
- Remaining TII manual batches: `278 / 306` still require fresh captcha sessions through the local operator.
