...
Updated at: `2026-06-08T12:01:39+08:00`
...
- Attempted TII manual batches: `32 / 306`
- Waiting on captcha: `1`
- Indexed TII manual batches: `31 / 306`
- Completed TII manual batches: `31 / 306`
- Partial TII manual batches: `0`
- Imported TII policy records: `46,013`
- TII official result rows before productId deduplication: `51,934`
- Official duplicate productId rows: `5,921`
- Saved TII detail pages: `45,846 / 46,013`
- Detail pages marked for later backfill: `167`
- Same-company same-name multi-product groups: `1,285` groups / `3,435` cards; these are preserved as separate cards because name-only deduplication would merge different versions.
- Completed TII batches: `tii-property-001` through `tii-property-031`.
- Latest completed TII batch: `tii-property-031` is complete with `1,746 / 1,746` official rows, `1,746` product cards, and `1,746 / 1,746` saved detail pages.
- Previous completed TII batch: `tii-property-030` is complete with `1,259 / 1,259` official rows, `1,259` product cards, and `1,226 / 1,259` saved detail pages; `33` official detail pages returned invalid detail responses and are marked for later backfill.
- Current waiting TII batch: `tii-property-032` is prepared and waiting for a fresh human-entered captcha.
- Remaining TII manual batches: `275 / 306` still require fresh captcha sessions through the local operator.
