...
from extract_tii_plan_benefits import (
    BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_B_PRODUCT_IDS,
    BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_B_VERSIONS,
    EXTRACTOR_VERSION,
    bnp_paribas_group_hospital_medical_d_semantic_phase,
    complete_strict_source_document,
    parse_bnp_paribas_group_hospital_medical_b_policy_state,
...
PROPOSAL_PATH = (
    ...
    / "tii-life-170-bnp-group-hospital-medical-rider-b-v254.json"
)
PARSER_ID = (
    "bnp-paribas-group-hospital-medical-rider-b-policy-state-v1"
)
...
def source_document(product_id: str) -> tuple[dict, Path]:
    version = BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_B_VERSIONS[
        product_id
    ]
...
def assert_invalid_schedule(schedule: dict, expected_error: str) -> None:
    try:
        validate_plan_options(
            schedule,
            "negative/tii-life-170/bnp-group-hospital-medical-b",
        )
    except SystemExit as error:
        assert expected_error in str(error), str(error)
    else:
        raise AssertionError(
            "formal validator accepted an invalid BNP Type B schedule"
        )
...
for product_id in sorted(
    ...
    source_version = BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_B_VERSIONS[
        product_id
    ]
    ...
    schedule = parse_bnp_paribas_group_hospital_medical_b_policy_state(
        document
    )
    ...
    assert version["coverage_type"] == "B"
    ...
    assert all(
        ...
        not in {
            "bnp-group-hospital-c-daily-additions",
...
wrong_cap = copy.deepcopy(schedules["267317R11A00200"])
...
wrong_type = copy.deepcopy(
    schedules["267313RZ1A00221A11Z10000015"]
)
wrong_type["version_characteristics"]["coverage_type"] = "C"
assert_invalid_schedule(wrong_type, "version formula is invalid")

wrong_hash = copy.deepcopy(schedules["267317R11A00200"])
wrong_hash["version_characteristics"][
    "source_document_sha256"
] = "0" * 64
assert_invalid_schedule(wrong_hash, "version formula is invalid")

bad_document, _ = source_document("267317R11A00200")
