...
PARSER_ID = "taiwan-cancer-insurance-unit-v1"
PROPOSAL_STEM = "tii-life-008-taiwan-cancer-insurance-v292"
PROPOSAL_PATH = (
...
SOURCE_MATRIX_PATH = (
    ...
    / "tii-life-008-taiwan-cancer-insurance.json"
)
...
def assert_invalid_schedule(schedule: dict, expected_error: str) -> None:
    try:
        validate_plan_options(
            schedule,
            "negative/tii-life-008/taiwan-cancer-insurance",
        )
...
for product_id in sorted(PRODUCT_IDS):
    ...
    assert version["maximum_units_stated_in_terms"] is False
    assert "maximum_units_per_insured" not in version
    assert version["hospital_daily_amount_per_unit"] == 1_600
    assert version["hospital_daily_day_limit"] is None
    assert version["posthumous_diagnosis_day_limit"] == 45
    ...
    assert version["claimant_identity_document_required"] is (
        revision >= 4
    )
    ...
    assert version["cancer_death_lump_sum_available"] is False
    assert version["home_recovery_benefit_available"] is False

    entries = {
        item["id"]: item for item in schedule["coverage_entries"]
    }
    assert set(entries) == expected_entry_ids
    assert entries["cancer-hospital-daily"]["amount"] == 1_600
    ...
    assert entries["posthumous-cancer-diagnosis-daily"][
        "quantity_cap"
    ] == 45
    assert entries["waiting-period-premium-refund"]["amount"] is None
    assert entries["precontract-unaware-cancer-premium-refund"][
        "amount"
    ] is None
...
document, _ = source_document("202321M11A68000")
for field, value in (
    ("batch_id", "tii-life-009"),
    ("document_type", "summary"),
    ("source_document_sha256", "0" * 64),
    ("file_name", "202321M11A68001-A.pdf"),
    ("product_id", "202321M11A68001"),
):
...
wrong_amount = copy.deepcopy(schedules["202321M11A68000"])
wrong_amount["coverage_entries"][0]["amount"] = 1_601
assert_invalid_schedule(wrong_amount, "exact entry contract is invalid")

wrong_cap = copy.deepcopy(schedules["202321M11A68000"])
wrong_cap["coverage_entries"][1]["quantity_cap"] = 44
assert_invalid_schedule(wrong_cap, "exact entry contract is invalid")

wrong_phase = copy.deepcopy(
    schedules["202321MZ1A68021A11Z10000008"]
)
...
wrong_cross_family = copy.deepcopy(document)
wrong_cross_family["product_id"] = "202321M11A68100"
wrong_cross_family["file_name"] = "202321M11A68100-A.pdf"
assert parse_policy(wrong_cross_family) is None
