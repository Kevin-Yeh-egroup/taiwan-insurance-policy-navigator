...
from extract_tii_plan_benefits import (
    BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_C_PRODUCT_IDS,
    BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_C_VERSIONS,
    EXTRACTOR_VERSION,
    bnp_paribas_group_hospital_medical_d_semantic_phase,
    complete_strict_source_document,
    parse_bnp_paribas_group_hospital_medical_c_policy_state,
...
PROPOSAL_PATH = (
    ...
    / "tii-life-170-bnp-group-hospital-medical-rider-c-v253.json"
)
PARSER_ID = (
    "bnp-paribas-group-hospital-medical-rider-c-policy-state-v1"
)
...
def source_document(product_id: str) -> tuple[dict, Path]:
    version = BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_C_VERSIONS[
        product_id
    ]
...
def assert_invalid_schedule(schedule: dict, expected_error: str) -> None:
    try:
        validate_plan_options(
            schedule,
            "negative/tii-life-170/bnp-group-hospital-medical-c",
        )
    except SystemExit as error:
        assert expected_error in str(error), str(error)
    else:
        raise AssertionError(
            "formal validator accepted an invalid BNP Type C schedule"
        )
...
for product_id in sorted(
    ...
    source_version = BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_C_VERSIONS[
        product_id
    ]
    ...
    schedule = parse_bnp_paribas_group_hospital_medical_c_policy_state(
        document
    )
    ...
    assert version["coverage_type"] == "C"
    ...
    assert all(
        entries[entry_id]["quantity_cap"] == 365
        for entry_id in capped_entry_ids
    )
    ...
    assert all(
        entry.get("benefit_group_id")
        != "bnp-group-hospital-d-daily-additions"
        for entry in entries.values()
    )
    schedules[product_id] = schedule


wrong_cap = copy.deepcopy(schedules["267317R11A00300"])
...
wrong_type = copy.deepcopy(
    schedules["267313RZ1A00321A11Z10000015"]
)
wrong_type["version_characteristics"]["coverage_type"] = "D"
assert_invalid_schedule(wrong_type, "version formula is invalid")

wrong_hash = copy.deepcopy(schedules["267317R11A00300"])
wrong_hash["version_characteristics"][
    "source_document_sha256"
] = "0" * 64
assert_invalid_schedule(wrong_hash, "version formula is invalid")

bad_document, _ = source_document("267317R11A00300")
