...
from extract_tii_plan_benefits import (
    BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_D_PRODUCT_IDS,
    BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_D_VERSIONS,
    EXTRACTOR_VERSION,
    bnp_paribas_group_hospital_medical_d_semantic_phase,
    complete_strict_source_document,
    parse_bnp_paribas_group_hospital_medical_d_policy_state,
...
PROPOSAL_PATH = (
    ...
    / "tii-life-170-bnp-group-hospital-medical-rider-d-v252.json"
)
PARSER_ID = (
    "bnp-paribas-group-hospital-medical-rider-d-policy-state-v1"
)
...
def source_document(product_id: str) -> tuple[dict, Path]:
    version = BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_D_VERSIONS[
        product_id
    ]
...
def assert_invalid_schedule(schedule: dict, expected_error: str) -> None:
    try:
        validate_plan_options(
            schedule,
            "negative/tii-life-170/bnp-group-hospital-medical-d",
        )
    except SystemExit as error:
        assert expected_error in str(error), str(error)
    else:
        raise AssertionError(
            "formal validator accepted an invalid BNP Type D schedule"
        )
...
for product_id in sorted(
    ...
    source_version = BNP_PARIBAS_GROUP_HOSPITAL_MEDICAL_D_VERSIONS[
        product_id
    ]
    ...
    schedule = parse_bnp_paribas_group_hospital_medical_d_policy_state(
        document
    )
    ...
    assert set(entries) == {
        ...
        "specified-surgery-medical-benefit",
        "post-discharge-convalescence-benefit",
        "pre-post-hospital-outpatient-benefit",
    }
    assert entries["general-hospital-daily-benefit"][
        "quantity_state_key"
    ] == "hospitalization_days"
    assert entries["cancer-hospital-daily-additional-benefit"][
        "quantity_state_key"
    ] == "cancer_hospitalization_days"
    assert entries["intensive-care-daily-additional-benefit"][
        "quantity_state_key"
    ] == "intensive_care_days"
    assert entries["burn-unit-daily-additional-benefit"][
        "quantity_state_key"
    ] == "burn_unit_days"
    assert entries["burn-unit-daily-additional-benefit"][
        "multiplier"
    ] == 2
    assert entries["specified-surgery-medical-benefit"][
        "multiplier"
    ] == 10
    assert entries["post-discharge-convalescence-benefit"][
        "rate_percent"
    ] == 50
...
wrong_phase = copy.deepcopy(schedules["267317R11A00405"])
wrong_phase["version_characteristics"][
    "newborn_screening_waiting_exception"
] = False
assert_invalid_schedule(wrong_phase, "version flag is invalid")

wrong_multiplier = copy.deepcopy(
    schedules["267313RZ1A00421A11Z10000015"]
)
wrong_multiplier["coverage_entries"][3]["multiplier"] = 1
assert_invalid_schedule(
    wrong_multiplier,
    "exact entry contract is invalid",
)

wrong_hash = copy.deepcopy(schedules["267317R11A00400"])
wrong_hash["version_characteristics"][
    "source_document_sha256"
] = "0" * 64
assert_invalid_schedule(wrong_hash, "version formula is invalid")

bad_document, _ = source_document("267317R11A00400")
