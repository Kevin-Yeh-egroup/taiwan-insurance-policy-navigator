...
def html_page(message: str = "") -> bytes:
    ...
    ok, prepare_message = ensure_captcha(batch_id) if batch_id else (False, "No pending batch.")
...
<head>
  ...
  <style>
    ...
    button {{ background: #155eef; color: white; border: 0; border-radius: 8px; font-weight: 800; }}
...
<main>
  ...
  <div class="panel">
    ...
    <form method="post" action="/submit">
      ...
      <label>人工輸入驗證碼 <input name="captcha" autocomplete="off" inputmode="numeric" required></label>
      <button type="submit">送出並抓完整批次</button>
    </form>
  </div>
  <div class="panel">
    <h2>訊息</h2>
