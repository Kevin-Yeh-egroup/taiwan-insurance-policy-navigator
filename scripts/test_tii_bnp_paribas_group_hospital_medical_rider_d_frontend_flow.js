...
const proposal = JSON.parse(
  fs.readFileSync(
    path.join(
      ROOT,
      "work/tii-benefit-proposals/tii-life-170-bnp-group-hospital-medical-rider-d-v252.json",
    ),
    "utf8",
  ),
);
...
for (const proposalItem of proposal.proposals) {
  ...
  assert.equal(
    model.policyStateRequirements(schedule).fields.find(
      (field) => field.key === "burn_unit_days",
    ).type,
    "integer",
  );
  assert.equal(
  ...
  const cappedHospital = valueFor(
    schedule,
    "general-hospital-daily-benefit",
    { ...policyState, hospitalization_days: 120 },
  );
  assert.equal(cappedHospital.value, 180_000);
  assert.equal(cappedHospital.eligible_quantity, 90);
  assert.equal(cappedHospital.quantity_cap, 90);

  const cappedOutpatient = valueFor(
    schedule,
    "pre-post-hospital-outpatient-benefit",
    { ...policyState, outpatient_visit_count: 30 },
  );
  assert.equal(cappedOutpatient.value, 10_500);
  assert.equal(cappedOutpatient.eligible_quantity, 21);
